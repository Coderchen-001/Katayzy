...
- Windows TensorRT：不打入主 release 包；RTX 20/30/40/50 用户由软件内 `KataGo 一键设置` 显式下载安装官方 KataGo `v1.17.0` `trt10.9.0-cuda12.8` 构建和所需运行库，并通过 NVIDIA GPU / Compute Capability 检测给出推荐状态；GTX 10 系及更老显卡优先 CUDA/OpenCL
